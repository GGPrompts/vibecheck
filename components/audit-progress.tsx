    ...
    return (
      <div className="space-y-2 p-3 rounded-lg bg-muted/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">Starting audit...</span>
        </div>
      </div>
    );
  }
      ...
      {moduleList.map((mod) => (
        <div key={mod.moduleId} className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            <span className="font-medium capitalize">{mod.moduleId.replace(/-/g, " ")}</span>
          ...
          {mod.message && (
            <p className="text-xs text-muted-foreground truncate">{mod.message}</p>
          )}
        </div>
      ))}
    </div>
